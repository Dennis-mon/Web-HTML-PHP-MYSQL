<?php include('headpublic.php');?>
    ...
    <section id="usuarioPublico">
        ...
        <?php
            ...
            echo '<label id="datos" class="lab">';
            echo '<h2> ' . $Usuario['NomUsuario'] . ' </h2>';
            echo '<img src="./' . $Usuario['Foto'] . '"  alt="foto">';
            echo '<p> Fecha de Incorporación: ' . $Usuario['FRegistro'] . ' </p>';
            echo '</label>';

            //Mostramos todos los albumes relacionados al usuario
            ...
            while( $Albumes = mysqli_fetch_assoc($resultado2)){
                echo '<label> <a href="datosAlbumPublico.php?Album=' . $Albumes['IdAlbum'] . '">Album ' . $Albumes['IdAlbum'] . ' </a></label>';
